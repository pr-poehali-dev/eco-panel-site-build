        ...
        <div className="grid lg:grid-cols-2 gap-12 max-w-5xl mx-auto">
          <div>
              ...
              {contactInfo.map((c) => (
                ...
                >
                  <div className="w-14 h-14 rounded-xl bg-primary/10 flex items-center justify-center shrink-0 group-hover:bg-primary transition-all duration-300">
                    <Icon name={c.icon} size={22} className="text-primary group-hover:text-white transition-colors duration-300" />
                  </div>
                  <div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-border">
              <h4 className="font-heading font-bold text-lg mb-2">Приезжайте к нам</h4>
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-8 space-y-5 shadow-sm border border-border">
            <Input
              placeholder="Ваше имя"
              value={form.name}
              onChange={(e) => setForm({ ...form, name: e.target.value })}
              required
              className="bg-secondary/50 border-border focus:border-primary focus:ring-primary/20 h-12"
            />
            <Input
              placeholder="Телефон"
              type="tel"
            ...
            />
            <Button type="submit" className="w-full bg-primary hover:bg-primary/90 text-white font-semibold h-14 text-base" size="lg">
